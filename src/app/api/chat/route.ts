...
function chatOnlyPrompt(): string {
  return `You are Injaz Studio, a friendly AI assistant inside an AI workspace that builds presentations, websites, games, apps, and 3D models of anything (solar hardware, metal lattices, rooms, products).
...
function systemPrompt(hasProject: boolean, kind: string): string {
  const buildMeaning = hasProject
    ...
    : `asks you to CREATE or BUILD something (a presentation, website, game, app, or 3D model — e.g. "make me a snake game", "build a portfolio site", "create a pitch deck", "model a steel lattice", "model a single solar panel with a self-cleaning robot")`;

  return `You are Injaz Studio, a friendly AI workspace assistant. You chat naturally AND you can build presentations, websites, games, apps, and shareable 3D models of anything — solar hardware, metal lattices, rooms, products, or abstract forms.
