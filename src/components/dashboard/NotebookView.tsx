...
import {
  ...
  Network, Table2, Gamepad2, ExternalLink, Sparkles, StickyNote,
} from "lucide-react";

/**
 * Notebook: a NotebookLM-style workspace. Add sources on the left, chat with
 * an AI grounded in them in the middle, and generate artifacts (decks,
 * quizzes, reports, mind maps...) from the sources on the right.
 */
...
const STORE_KEY = "studio-notebook-v1";
...
export function NotebookView() {
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem(
        STORE_KEY,
        JSON.stringify({ sources, messages: messages.slice(-40), outputs: outputs.slice(-20) })
      );
    } catch {
      /* storage full — keep the session in memory */
    }
  }, [sources, messages, outputs, hydrated]);

  // Absorb finished uploads into the sources list.
  useEffect(() => {
  ...
  async function send(textOverride?: string) {
    ...
    setChatting(true);
    try {
      const history = [...messages, userMsg].slice(-12).map((m) => ({ role: m.role, text: m.text }));
      ...
      if (decision.mode === "chat") {
        setMessages((m) => [...m, { id: sid(), role: "assistant", text: decision.reply }]);
      } else {
        setChatting(false);
  ...
  async function runGeneration(
    ...
    try {
      const id = await generateProject(
        {
          prompt,
          kind,
          files: sources.map((s) => ({ name: s.name, kind: s.kind, content: s.content })),
        },
      ...
      setOutputs((o) => [output, ...o]);
      setMessages((m) => [
        ...m,
        { id: sid(), role: "assistant", text: `Done — your ${label.toLowerCase()} is ready in Studio outputs on the right.` },
      ]);
    } catch (e) {
  ...
  function addPastedText() {
    ...
    setSources((prev) => [
      ...
      {
        id: sid(),
        name: pasteName.trim() || `Pasted text ${prev.length + 1}`,
        kind: "text",
